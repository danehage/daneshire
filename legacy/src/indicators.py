...
class TechnicalIndicators:
    ...
    @staticmethod
    def calculate_volume_pace(current_volume: int, avg_volume: int) -> tuple[float, bool]:
        """
        ...
        Example: At 10:30 AM with 3M volume vs 10M average:
        - Expected: 10M * 15% = 1.5M
        - Pace: 3M / 1.5M = 2.0x (actually high!)
        """
        if avg_volume <= 0:
            return 0.0, False
    ...
    @staticmethod
    def calculate_avg_volume(df: pd.DataFrame, days: int = 20) -> int | None:
    ...
    def calculate_rsi(df: pd.DataFrame, period: int = 14) -> float:
        ...
        Uses exponential moving average (Wilder's method) for accurate RSI
        that matches standard trading platforms.
        """
    ...
    @staticmethod
    def calculate_volatility_rank(df: pd.DataFrame) -> dict | None:
        """
        ...
        Use as a proxy when IV data is unavailable.

        Returns:
            dict with 'rank' (0-100), 'current_hv', and 'is_proxy' flag
        """
        ...
        vol_52w = rolling_vol.tail(TRADING_DAYS_PER_YEAR) if len(rolling_vol) >= TRADING_DAYS_PER_YEAR else rolling_vol
        ...
        return {
            ...
            'is_proxy': True  # Always true since we don't have real IV data
        }

    @staticmethod
    def calculate_trend(current_price: float, df: pd.DataFrame) -> dict | None:
        """
        Classify trend based on moving averages and price position.

        Returns trend classification, distance from MAs, and MA values.
        """
        ...
        # Calculate MA slope - need at least 60 rows for reliable slope
        ma_slope = 0.0
        if len(df) >= 60:
    ...
    def calculate_52w_position(current_price: float, df: pd.DataFrame) -> dict:
        ...
        Below 40% indicates potential value zone.
        """
    ...
    def calculate_support_resistance(current_price: float, df: pd.DataFrame) -> dict:
        ...
        # Recent highs/lows (20 and 60 day)
    ...
    @staticmethod
    def calculate_momentum(current_price: float, current_volume: int, df: pd.DataFrame) -> dict:
        """
        Calculate momentum indicators.

        Returns volume ratio, RSI, and 10-day momentum.
        """
        if df is None or len(df) < 15:  # Need enough data for RSI
            return {
                'volume_ratio': 1.0,
                'rsi': 50.0,
                'momentum_10d': 0.0
        ...
        momentum_10d = ((current_price - price_ago) / price_ago) * 100 if price_ago > 0 else 0.0
