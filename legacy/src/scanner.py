...
Main scanning logic with caching and parallel processing
"""
...
class StockScanner:
    """
    Institutional-grade stock scanner for swing trading and options strategies.
    ...
    def _load_cache(self, cache_name: str) -> dict | None:
    ...
    def _process_ticker(self, ticker: str, quote: dict) -> dict | None:
        """
        ...
        """
        try:
            hist_data = self.client.get_historical_data(ticker, days=252)
            ...
            momentum = self.indicators.calculate_momentum(current_price, current_volume, df)
    ...
    def _score_opportunity(self, result: dict) -> tuple[int, list[str]]:
        ...
        Higher scores = better opportunity for swing trades / option selling.
        """
        score = 0
        signals = []

        # High HV Rank - good for option selling (volatility elevated)
        ...
        if range_pos < 25:
            ...
            score -= 10  # Near highs, less upside
        ...
        # Liquidity bonus (easier to trade)
    ...
    def run_scan(
        ...
        Returns:
            List of analyzed stocks sorted by score (highest first)
        """
        # Stage 1: Fast filter
        survivors = {}
        ...
        if not survivors:
            ...
            all_quotes = self.client.get_batch_quotes(tickers)
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = {
                executor.submit(self._process_ticker, ticker, quote): ticker
                for ticker, quote in survivors.items()
            }
            ...
            for future in as_completed(futures):
                ...
                if completed % 20 == 0:
                    logger.info(f"Processed {completed}/{total} - {len(results)} valid, {errors} errors")
    ...
    def analyze_ticker(self, ticker: str) -> dict | None:
        ...
        result = self._process_ticker(ticker, quote)
        if result:
            score, signals = self._score_opportunity(result)
            result['score'] = score
            result['signals'] = signals
            result['change'] = quote.get('change', 0)
            result['change_pct'] = quote.get('changesPercentage', quote.get('changePercentage', 0))
